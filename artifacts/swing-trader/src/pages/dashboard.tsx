  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Real-time system overview and aggregated signals.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard 
          ...
          subtitle={summary?.totalPnl && summary.totalPnl > 0 ? "Profitable" : "Loss"}
          trend={summary?.totalPnl && summary.totalPnl > 0 ? "up" : "down"}
          icon={Activity}
        />
        <StatCard 
          title="Win Rate" 
          value={isLoadingSummary ? null : `${(summary?.winRate || 0) * 100}%`}
          subtitle="All time"
          icon={Target}
        />
        <StatCard 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-lg font-semibold tracking-tight">Top Swing Signals</h2>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-secondary/50 text-muted-foreground font-data text-xs border-b border-border">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium">RSI / MACD</th>
              ...
              <tbody className="divide-y divide-border">
                {isLoadingSignals ? (
                  ...
                  <tr key={signal.symbol} className="hover:bg-secondary/20 transition-colors">
                    <td className="px-4 py-3 font-bold">{signal.symbol}</td>
                    <td className="px-4 py-3 text-right font-data">${signal.price.toFixed(2)}</td>
                    <td className={cn(
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/chart/${signal.symbol}`} className="text-xs bg-secondary hover:bg-primary hover:text-primary-foreground transition-colors px-3 py-1.5 rounded text-foreground font-medium">
                        Chart
                      </Link>
                    </td>
                  </tr>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-semibold tracking-tight">Movers</h2>
          <div className="space-y-4">
            <Card className="bg-card">
              <CardHeader className="py-4">
                <CardTitle className="text-sm text-muted-foreground">Top Gainer</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  summary?.topGainer ? (
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <div className="font-data text-bullish text-lg">+{summary.topGainer.changePercent.toFixed(2)}%</div>
                        <div className="font-data text-muted-foreground">${summary.topGainer.price.toFixed(2)}</div>
                      </div>
                    </div>
            ...
            <Card className="bg-card">
              <CardHeader className="py-4">
                <CardTitle className="text-sm text-muted-foreground">Top Loser</CardTitle>
              </CardHeader>
              <CardContent>
                {isLoadingSummary ? <Skeleton className="h-10 w-full" /> : (
                  summary?.topLoser ? (
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <div className="font-data text-bearish text-lg">{summary.topLoser.changePercent.toFixed(2)}%</div>
                        <div className="font-data text-muted-foreground">${summary.topLoser.price.toFixed(2)}</div>
                      </div>
                    </div>
                  ) : <div className="text-muted-foreground text-sm">No data</div>
                )}
      ...
      <CardContent className="p-4 md:p-6 flex flex-col justify-between h-full">
        ...
        <div>
          {value === null ? (
            ...
            <div className={cn(
              "text-2xl md:text-3xl font-bold tracking-tight font-data",
...
export function SignalBadge({ signal }: { signal: string }) {
  const mapping: Record<string, { label: string, cls: string }> = {
    'strong_buy': { label: 'STRONG BUY', cls: 'bg-bullish' },
    'buy': { label: 'BUY', cls: 'bg-bullish opacity-80' },
    'neutral': { label: 'NEUTRAL', cls: 'bg-secondary text-muted-foreground border border-border' },
    'sell': { label: 'SELL', cls: 'bg-bearish opacity-80' },
    'strong_sell': { label: 'STRONG SELL', cls: 'bg-bearish' },
  };

  const config = mapping[signal] || mapping.neutral;

  return (
    <span className={cn("px-2 py-0.5 rounded text-[10px] font-bold tracking-wider", config.cls)}>
      {config.label}
    </span>
  );
}
